...
RegisterNetEvent('az-vmenu-qb:client:playerAction', function(action)
...
RegisterNetEvent('vmenu_ui:qbPlayerAction', function(action)
    TriggerEvent('az-vmenu-qb:client:playerAction', action)
end)

RegisterNetEvent('az-vmenu-qb:client:applySpawnedVehicle', function(netId, plate, modelHash, vehicleClass, cost)
    debugPrint('spawned vehicle', 'netId=', netId, 'plate=', plate, 'model=', modelHash, 'class=', vehicleClass, 'cost=', cost)

end)

RegisterNetEvent('vMenu:QBCore:PermissionsRefreshed', function()
    debugPrint('permissions refreshed')
end)

CreateThread(function()
    Wait(2500)
    TriggerServerEvent('vMenu:QBCore:RequestPlayers')
    debugPrint('AZ-ONLY client bridge loaded')
end)
