...
state = {
  serverName = GetConvar('sv_hostname', 'vMenu'),
  ...
  ui = {
    ...
    brandText = (Config and Config.UI and Config.UI.brandText) or 'vMenu',
...
local function applyQBCoreSpawnedVehicle(vehicle, plate, cost)
  ...
  if cost > 0 then
    qbNotify(('vMenu vehicle spawned. Keys assigned. Charged $%s.'):format(cost), 'success')
  elseif plate ~= '' then
    qbNotify('vMenu vehicle spawned. Keys assigned.', 'success')
  end
end

RegisterNetEvent('vmenu_ui:qbPlayerAction', function(action)
...
RegisterNetEvent('vmenu_ui:qbForceServerAcquire', function(plate)
  local keys = qbConfig().Keys or {}
  if keys.UseServerAcquireEvent and keys.ServerAcquireEvent and keys.ServerAcquireEvent ~= '' then
...
local function loadPersistentState()
  state.savedVehicles = loadJsonKvp('vmenu_ui_saved_vehicles')
  state.savedVehicleCategories = loadJsonKvp('vmenu_ui_saved_vehicle_categories')
  if type(state.savedVehicleCategories) ~= 'table' then state.savedVehicleCategories = {} end
  state.savedPeds = loadJsonKvp('vmenu_ui_saved_peds')
  state.savedOutfits = loadJsonKvp('vmenu_ui_saved_outfits')
  state.loadouts = loadJsonKvp('vmenu_ui_loadouts')
  state.blockedPlayers = loadJsonKvp('vmenu_ui_blocked_players')

  local ui = loadJsonKvp('vmenu_ui_ui_settings')
  ...
  local toggleSettings = loadJsonKvp('vmenu_ui_toggle_settings')
...
local function savePersistentState()
  saveJsonKvp('vmenu_ui_saved_vehicles', state.savedVehicles)
  saveJsonKvp('vmenu_ui_saved_vehicle_categories', state.savedVehicleCategories or {})
  saveJsonKvp('vmenu_ui_saved_peds', state.savedPeds)
  saveJsonKvp('vmenu_ui_saved_outfits', state.savedOutfits)
  saveJsonKvp('vmenu_ui_loadouts', state.loadouts)
  saveJsonKvp('vmenu_ui_blocked_players', state.blockedPlayers or {})
  saveJsonKvp('vmenu_ui_ui_settings', {
  ...
  saveJsonKvp('vmenu_ui_toggle_settings', {
...
local function hasImportedLegacyVehicle(legacyKey)
  ...
  for _, entry in ipairs(state.savedVehicles or {}) do
    if type(entry) == 'table' and tostring(entry.legacyVMenuKey or '') == legacyKey then
      return true
    end
  end
  return false
...
local function convertLegacyVMenuVehicle(legacyKey, legacy)
  ...
  local out = {
    version = 1,
    kind = 'vehicle',
    importedFrom = 'TomGrobbe vMenu .dll KVP',
    legacyVMenuKey = tostring(legacyKey or ''),
...
local function importLegacyVMenuSavedVehicles(forceNotify)
  local imported, skipped, failed = 0, 0, 0
  local prefix = (Config and Config.LegacyVMenu and Config.LegacyVMenu.vehiclePrefix) or 'veh_'
  local handle = StartFindKvp(prefix)
  if handle == -1 or handle == nil then
    return true, 'No old vMenu saved vehicles were found', { imported = 0, skipped = 0, failed = 0 }
  end
  ...
  while true do
    ...
    if hasImportedLegacyVehicle(key) then
      ...
      local converted = ok and convertLegacyVMenuVehicle(key, legacy) or nil
      if converted then
  ...
  local msg = ('Legacy .dll vMenu import: %s imported, %s already imported, %s failed.'):format(imported, skipped, failed)
...
awaitServer = function(action, payload)
  ...
  local p = promise.new()
  pending[reqId] = p
  TriggerServerEvent('vmenu_ui:serverAction', reqId, action, payload or {})
  return Citizen.Await(p)
end

RegisterNetEvent('vmenu_ui:serverResponse', function(id, response)
  if pending[id] then pending[id]:resolve(response); pending[id] = nil end
end)
RegisterNetEvent('vmenu_ui:teleportToCoords', function(coords) setCoordsSafe(coords.x, coords.y, coords.z) end)
RegisterNetEvent('vmenu_ui:killMe', function() SetEntityHealth(ped(), 0) end)
RegisterNetEvent('vmenu_ui:privateMessage', function(fromName, msg, fromId)
...
RegisterNetEvent('vmenu_ui:gpsRequest', function(requester, requesterName, expiresAt)
  ...
  notify(('%s requested your GPS location. Open vMenu > Civilian Player Menu > GPS Requests to accept or deny.'):format(requesterName or ('ID ' .. requester)))
end)

RegisterNetEvent('vmenu_ui:gpsRequestClosed', function(requester)
...
RegisterNetEvent('vmenu_ui:gpsAccepted', function(targetName, coords)
...
RegisterNetEvent('vmenu_ui:playerEvent', function(kind, playerName)
...
RegisterNetEvent('vmenu_ui:syncWorld', function(world)
...
local function handleAction(action, value, context)
  ...
  elseif action == 'importLegacyVMenuVehicles' then return importLegacyVMenuSavedVehicles(true)
  ...
  elseif action == 'setBrandText' then
    ...
    if brand == '' then brand = (Config and Config.UI and Config.UI.brandText) or 'vMenu' end
  ...
  elseif action == 'resetMenuAppearance' then
    ...
    if state.ui.allowBannerEditing ~= false then
      state.ui.brandText = (Config and Config.UI and Config.UI.brandText) or 'vMenu'
...
RegisterCommand('vmenuui', function() setUi(not uiOpen) end, false)
RegisterKeyMapping('vmenuui', 'Open Styled vMenu UI', 'keyboard', 'M')
RegisterCommand('vmenuui_noclip', function() handleToggle('noclip') end, false)
RegisterKeyMapping('vmenuui_noclip', 'Toggle vMenu UI NoClip', 'keyboard', 'F2')
...
CreateThread(function()
  ...
  if not (Config and Config.LegacyVMenu and Config.LegacyVMenu.autoImportSavedVehicles == false) then
    importLegacyVMenuSavedVehicles(false)
  end
  Wait(200)
