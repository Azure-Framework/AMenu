(() => {
  ...
  const baseMenus = {
    ...
    about: () => ({
      title: 'About',
      items: [
        { label: 'Custom vMenu-style UI', type: 'info', description: 'This build uses a live NUI plus client/server Lua actions.' },
        { label: 'Open Keybind', type: 'info', description: 'Press M or use /vmenuui.' },
    ...
    vehicleOptions: (ctx, snap) => ({
      ...
      items: [
        ...
        arrowEntry('Vehicle Colors', 'vehicleColors', 'Style your vehicle even further with a full paint menu like vMenu.'),
    ...
    savedVehicles: (ctx, snap) => ({
      ...
      items: [
        ...
        { label: 'Import Old .dll vMenu Vehicles', type: 'action', action: 'importLegacyVMenuVehicles', description: 'Pulls old TomGrobbe/vMenu saved vehicles from client KVP entries named veh_*. Safe to press more than once.' },
        ...((snap.savedVehicles || []).length
          ? snap.savedVehicles.flatMap((vehicle, index) => ([
              { label: `Spawn ${vehicle.name}`, type: 'action', action: 'spawnSavedVehicle', value: index, description: `${vehicle.model || vehicle.label || 'saved vehicle'}${vehicle.legacyCategory ? ' | Old vMenu category: ' + vehicle.legacyCategory : ''}` },
    ...
    worldOptions: (ctx, snap) => ({
      ...
      items: (snap.config?.worldControlsEnabled === false)
        ...
        : [
            { label: 'Time Options', submenu: 'timeOptions', description: 'Synced server time controls like vMenu.' },
            { label: 'Weather Options', submenu: 'weatherOptions', description: 'Synced server weather controls like vMenu.' }
          ]
    }),
...
weatherOptions: () => ({
  ...
  items: [
    ...
    { label: 'Remove Clouds', type: 'action', action: 'removeClouds', description: 'Clear cloud hats like vMenu.' },
    { label: 'Randomize Clouds', type: 'action', action: 'randomizeClouds', description: 'Pick a random cloud pattern like vMenu.' },
    ...
    miscSettingsLegacy: (ctx, snap) => ({
      ...
      items: [
        ...
        { label: 'About', submenu: 'about', description: 'Information about this custom vMenu-style resource.' },
    ...
    menuSettings: (ctx, snap) => ({
      ...
      items: [
        ...
        ...(snap.ui?.allowBannerEditing !== false ? [
          ...
          { label: 'Set Brand Text', type: 'prompt', action: 'setBrandText', description: 'Set the title text shown in the menu header.', fields: [{ name: 'text', label: 'Brand text', value: snap.ui?.brandText || 'vMenu' }] },
        ...
        { label: 'Current Brand Text', type: 'info', description: snap.ui?.brandText || 'vMenu' },
    ...
    qbcoreManagement: (ctx, snap) => {
      ...
      if (!qb.enabled) {
        return { title: fw.upper, items: [{ label: 'Framework Bridge Disabled', type: 'info', description: 'Enable the vMenu framework bridge config.' }] };
      }
      if (!qb.coreStarted) {
        return { title: fw.upper, items: [{ label: 'No Active Framework Started', type: 'info', description: 'Start Az-Framework before vMenu-Bridge/vMenu, then re-select your character or restart the server.' }] };
      }
      if (!qb.canAccessMenu) {
        return { title: fw.upper, items: [{ label: 'No Permission', type: 'info', description: 'Grant vMenu.Framework.Admin, vMenu.QBCore.Menu, vMenu.QBCore.All, vMenu.QBCore.Admin, vMenu.Staff, or framework admin permission.' }] };
      }
    ...
    permissionsEditor: (ctx, snap) => ({
      ...
      items: [
        ...
        { label: 'Add ACE Rule', type: 'prompt', action: 'addPermissionAce', description: 'Add a permissions.cfg style add_ace rule.', fields: [{ name: 'principal', label: 'Principal (group.admin, builtin.everyone, id)', value: 'group.admin' }, { name: 'ace', label: 'ACE permission', value: 'vMenu.NoClip' }, { name: 'mode', label: 'allow / deny', value: 'allow' }] },
        { label: 'Remove ACE Rule', type: 'prompt', action: 'removePermissionAce', description: 'Remove a permissions.cfg style add_ace rule.', fields: [{ name: 'principal', label: 'Principal (group.admin, builtin.everyone, id)', value: 'group.admin' }, { name: 'ace', label: 'ACE permission', value: 'vMenu.NoClip' }, { name: 'mode', label: 'allow / deny', value: 'allow' }] },
  ...
  const savedVehicleDescription = (ctx = {}) => `${ctx.vehicleModel || 'Saved vehicle'} | ${ctx.vehicleClass || 'Unknown'} | ${ctx.category || 'Uncategorized'}${ctx.legacyCategory ? ' | Old vMenu category: ' + ctx.legacyCategory : ''}${ctx.available === false ? ' | model unavailable/missing' : ''}`;
  const savedVehicleRow = (vehicle, fallbackIndex = 0) => {
    const ctx = savedVehicleContext(vehicle, fallbackIndex);
    return { label: `${ctx.vehicleName}${ctx.importedFrom ? '  • old vMenu' : ''}${ctx.available === false ? '  • unavailable' : ''}`, submenu: 'savedVehicleActions', context: ctx, description: savedVehicleDescription(ctx) };
  };
  ...
  Object.assign(baseMenus, {
    main: (ctx, snap) => {
      ...
      items: [
        ...
        arrowEntry('Misc Settings', 'miscSettings', 'Miscellaneous vMenu options/settings can be configured here. You can also save your settings in this menu.'),
        arrowEntry('About vMenu', 'about', 'Information about vMenu.')
      ]
      };
    },
    ...
    about: (ctx, snap) => ({
      ...
      items: [
        ...
        { label: 'vMenu Version', type: 'info', description: 'Lua/NUI recreation matching the uploaded reference structure and flow.' },
        { label: 'About vMenu / Credits', type: 'info', description: 'Reference structure based on the original vMenu layout by Vespura.' }
      ]
    }),
    ...
    playerAppearance: () => ({
      ...
      items: [
        ...
        { label: 'Use Server Clothing Resource', type: 'info', description: 'For serious RP, clothing edits should stay inside your clothing/appearance resource. vMenu only saves and loads ped/character snapshots.' }
      ]
    }),
    ...
    savedVehicles: (ctx, snap) => ({
      ...
      items: [
        ...
        ...(!(snap.savedVehicles || []).length ? [{ label: 'No saved vehicles', type: 'info', description: 'Use Save Current Vehicle first, or import your old .dll vMenu saves.' }] : [])
      ]
    }),
    ...
    weatherOptions: () => ({
      ...
      items: [
        ...
        { label: 'Remove All Clouds', type: 'action', action: 'removeClouds', description: 'Clear cloud hats like vMenu.' },
        { label: 'Randomize Clouds', type: 'action', action: 'randomizeClouds', description: 'Pick a random cloud pattern like vMenu.' }
      ]
    }),
  ...
  function registeredCommands(snap) {
    ...
    return commands
      ...
      .map(cmd => {
        const name = String(cmd.command || cmd.name || cmd.label || '').replace(/^\//, '').trim();
  ...
  function applyUiSettings() {
    ...
    brandText.textContent = preset.brandText || ui.brandText || 'vMenu';
  ...
  function persistNavState() {
    syncSelectedToStack();
    try { localStorage.setItem('vmenu_ui_last_stack', JSON.stringify(state.stack || [{ id: 'main', selected: 0 }])); } catch (e) {}
  }

  function loadNavState() {
    try {
      const raw = localStorage.getItem('vmenu_ui_last_stack');
