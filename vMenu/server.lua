...
local function broadcastWorldState(target)
  if target then
    TriggerClientEvent('vmenu_ui:syncWorld', target, worldState)
  else
    TriggerClientEvent('vmenu_ui:syncWorld', -1, worldState)
  end
end

local function requireWorldAce(src)
  return hasAce(src, 'vMenu.TimeOptions.Menu') or hasAce(src, 'vMenu.TimeOptions.All') or hasAce(src, 'vMenu.TimeOptions.SetTime') or hasAce(src, 'vMenu.TimeOptions.FreezeTime') or hasAce(src, 'vMenu.WeatherOptions.Menu') or hasAce(src, 'vMenu.WeatherOptions.All') or hasAce(src, 'vMenu.WeatherOptions.SetWeather') or hasAce(src, 'vMenu.WeatherOptions.Dynamic') or hasAce(src, 'vMenu.WeatherOptions.Blackout') or hasAce(src, 'vMenu.WeatherOptions.RemoveClouds') or hasAce(src, 'vMenu.WeatherOptions.RandomizeClouds') or hasAce(src, 'vMenu.Staff')
end
...
local function parsePermissionsFile(content)
  ...
  for line in (content .. '\n'):gmatch('(.-)\r?\n') do
    ...
    if cleaned == '' or cleaned:match('^#') or cleaned:match('^//') then
      if cleaned ~= '# vMenu UI permissions rules' then
...
local function savePermissionsData()
  ...
  table.insert(lines, '# vMenu UI permissions rules')
...
local function canEditPermissions(src)
  return hasAce(src, 'vMenu.Permissions.Editor') or hasAce(src, 'vMenu.Staff')
end
...
AddEventHandler('playerJoining', function()
  ...
  TriggerClientEvent('vmenu_ui:playerEvent', -1, 'joined', joinName)
...
AddEventHandler('playerDropped', function()
  ...
  for target, list in pairs(pendingGpsRequests) do
    if list[src] then
      list[src] = nil
      TriggerClientEvent('vmenu_ui:gpsRequestClosed', target, src)
    end
  end
  TriggerClientEvent('vmenu_ui:playerEvent', -1, 'left', leftName)
end)
...
local function frameworkBridgeResources()
  local configured = (Config and Config.QBCore and Config.QBCore.FrameworkBridgeResource) or 'vMenu-Bridge'
  local list, seen = {}, {}
  ...
  add(configured)
  add('vMenu-Bridge')
  add('az_vmenu_framework_bridge')
  add('az_vmenu_qbcore_bridge')
  return list
end
...
local function callFrameworkBridgeExport(exportName, ...)
  ...
  for _, resource in ipairs(frameworkBridgeResources()) do
    if GetResourceState(resource) == 'started' then
      ...
      elseif not ok then
        print(('[vMenu NUI FrameworkBridge] Colon export %s failed on %s: %s'):format(tostring(exportName), tostring(resource), tostring(result)))
        ...
        elseif not ok2 then
          print(('[vMenu NUI FrameworkBridge] Dot export %s failed on %s: %s'):format(tostring(exportName), tostring(resource), tostring(resultA)))
        end
      end
    end
  end
...
local function qbHasAdmin(src)
  ...
  local cfg = qbConfig()
  if IsPlayerAceAllowed(src, cfg.AdminAce or 'vMenu.QBCore.Admin') then return true end
  if IsPlayerAceAllowed(src, 'vMenu.QBCore.Menu') then return true end
  if IsPlayerAceAllowed(src, 'vMenu.QBCore.All') then return true end
  if IsPlayerAceAllowed(src, 'vMenu.Staff') then return true end
  local core = qbLoadCore()
...
local function azFirstExport(names, ...)
  for _, name in ipairs(names or {}) do
    ...
    if Config and Config.Debug then
      print(('[vMenu Az Direct] export failed %s: %s'):format(tostring(name), tostring(err)))
    end
  end
...
local function azDirectPlayerAction(staffSrc, payload)
  ...
  do
    ...
    print(('[vMenu AZ] Sending direct bridge action staff=%s action=%s target=%s args=%s'):format(
    ...
    print(('[vMenu AZ] Direct bridge export failed or returned nil for action=%s target=%s'):format(tostring(action), tostring(target)))
  end
  ...
  if action == 'setjob' then
    ...
    if ok and result == true then
      ...
      TriggerEvent('vMenu:QBCore:RefreshPermissions', target)
  ...
  if action == 'revive' then
    TriggerClientEvent('vmenu_ui:qbPlayerAction', target, 'revive')
    TriggerClientEvent('az-vmenu-qb:client:playerAction', target, 'revive')
    return { ok = true, message = 'Az revive sent.' }
  end

  if action == 'heal' then
    TriggerClientEvent('vmenu_ui:qbPlayerAction', target, 'heal')
    TriggerClientEvent('az-vmenu-qb:client:playerAction', target, 'heal')
  ...
  if action == 'duty' then
    return { ok = true, message = 'Az duty accepted. Az uses active department, not QBCore duty.' }
  end
...
local function qbExecutePlayerAction(src, actionName, target, args)
  ...
  if src ~= 0 and not qbHasAdmin(src) then return { ok = false, message = 'No QBCore management permission' } end
  local Target = qbGetPlayer(target)
  if not Target then return { ok = false, message = ('Target not found/loaded in QBCore: %s'):format(tostring(target)) } end
  ...
  if actionName == 'info' then
    ...
    return { ok = true, message = 'QBCore player info opened', title = 'QBCore Player Info', displayText = msg }
  ...
  elseif actionName == 'addmoney' or actionName == 'removemoney' then
    ...
    local ok
    if actionName == 'addmoney' then ok = Target.Functions.AddMoney(account, amount, 'vMenu NUI QBCore management')
    else ok = Target.Functions.RemoveMoney(account, amount, 'vMenu NUI QBCore management') end
  ...
  elseif actionName == 'keys' then
    ...
    if keys.UseServerAcquireEvent and keys.ServerAcquireEvent and keys.ServerAcquireEvent ~= '' then
      TriggerClientEvent('vmenu_ui:qbForceServerAcquire', target, plate)
    end
...
RegisterNetEvent('vmenu_ui:serverAction', function(requestId, action, payload)
  local src = source
  loadBans()
  cleanupBans()

  local function respond(data)
    TriggerClientEvent('vmenu_ui:serverResponse', src, requestId, data)
  end
  ...
  if action == 'qbCanSpawnVehicle' then
    ...
    if bridgeAllowedOk then
      if bridgeAllowed ~= true then
        local msg = 'You do not have active-framework access to the vMenu vehicle spawner.'
        if bridgeReason == 'offduty' then msg = 'You must be on duty to use the vMenu vehicle spawner.'
        elseif bridgeReason and bridgeReason ~= '' then msg = ('Your job/group (%s) cannot use the vMenu vehicle spawner.'):format(bridgeReason) end
        return respond({ ok = false, cost = 0, message = msg })
      end
      local costOk, cost = callFrameworkBridgeExport('GetSpawnCost', src, vehicleClass)
      cost = tonumber(cost) or 0
      local chargeOk, paid, account = callFrameworkBridgeExport('ChargePlayer', src, cost, ('vMenu NUI vehicle spawn class %s'):format(tostring(vehicleClass)))
    ...
    if not allowed then
      local msg = 'You do not have QBCore access to the vMenu vehicle spawner.'
      if reason == 'offduty' then msg = 'You must be on duty to use the vMenu vehicle spawner.'
      elseif reason and reason ~= '' then msg = ('Your job (%s) cannot use the vMenu vehicle spawner.'):format(reason) end
      return respond({ ok = false, cost = 0, message = msg })
    end
    local cost = qbGetSpawnCost(src, Player, vehicleClass)
    local paid, account = qbRemoveMoney(Player, cost, ('vMenu NUI vehicle spawn class %s'):format(tostring(vehicleClass)))
  ...
  if action == 'getBans' then
    if not hasAce(src, 'vMenu.OnlinePlayers.ViewBannedPlayers') and not hasAce(src, 'vMenu.Staff') then return respond({ ok = false, bans = {}, message = 'No permission' }) end
    return respond({ ok = true, bans = bans, message = 'Bans loaded' })
  end

  if action == 'unbanPlayer' then
    if not hasAce(src, 'vMenu.OnlinePlayers.Unban') and not hasAce(src, 'vMenu.Staff') then return respond({ ok = false, message = 'No permission' }) end
  ...
  if action == 'identifiers' then
    if not hasAce(src, 'vMenu.OnlinePlayers.Identifiers') and not hasAce(src, 'vMenu.Staff') then return respond({ ok = false, message = 'No permission' }) end
    local target = tonumber(payload.target)
    if not target or not GetPlayerName(target) then return respond({ ok = false, message = 'Player not found' }) end
  ...
  if action == 'civRequestGPS' then
    ...
    TriggerClientEvent('vmenu_ui:gpsRequest', target, src, getPlayerNameSafe(src), expiresAt)
    qbNotify(target, ('%s requested your GPS location. Open vMenu > Civilian Player Menu > GPS Requests.'):format(getPlayerNameSafe(src)), 'primary', 8000)
  ...
  if action == 'civRespondGPS' then
    ...
    TriggerClientEvent('vmenu_ui:gpsRequestClosed', src, requester)
    ...
    TriggerClientEvent('vmenu_ui:gpsAccepted', requester, getPlayerNameSafe(src), { x = coords.x, y = coords.y, z = coords.z })
  ...
  if action == 'civPrivateMessage' then
    ...
    TriggerClientEvent('vmenu_ui:privateMessage', target, getPlayerNameSafe(src), message, src)
    TriggerClientEvent('vmenu_ui:privateMessage', src, 'To ' .. getPlayerNameSafe(target), message, target)
  ...
  if action == 'civGiveMoney' then
    ...
    local bridgeRemoveOk, bridgeRemoved = callFrameworkBridgeExport('RemoveMoney', src, account, amount, ('vMenu civilian transfer to %s'):format(target))
    if bridgeRemoveOk then
      ...
      local bridgeAddOk, bridgeAdded = callFrameworkBridgeExport('AddMoney', target, account, amount, ('vMenu civilian transfer from %s'):format(src))
      if bridgeAddOk and bridgeAdded == true then
        TriggerClientEvent('vmenu_ui:privateMessage', target, 'Money Transfer', ('%s gave you $%s %s.'):format(getPlayerNameSafe(src), amount, account), src)
        return respond({ ok = true, message = ('Gave $%s %s to %s'):format(amount, account, getPlayerNameSafe(target)) })
      end
      callFrameworkBridgeExport('AddMoney', src, account, amount, 'vMenu civilian transfer refund')
    ...
    local removed = Sender.Functions.RemoveMoney(account, amount, ('vMenu civilian transfer to %s'):format(target))
    if removed ~= true then return respond({ ok = false, message = ('You do not have enough %s money.'):format(account) }) end
    local added = Target.Functions.AddMoney(account, amount, ('vMenu civilian transfer from %s'):format(src))
    if added ~= true then
      Sender.Functions.AddMoney(account, amount, 'vMenu civilian transfer refund')
      return respond({ ok = false, message = 'Transfer failed and was refunded' })
    end
  ...
  if action == 'sendPrivateMessage' then
    if not hasAce(src, 'vMenu.OnlinePlayers.SendMessage') and not hasAce(src, 'vMenu.Staff') then return respond({ ok = false, message = 'No permission' }) end
    local target = tonumber(payload.target)
  ...
  if action == 'summonPlayer' then
    if not hasAce(src, 'vMenu.OnlinePlayers.Summon') and not hasAce(src, 'vMenu.Staff') then return respond({ ok = false, message = 'No permission' }) end
    local target = tonumber(payload.target)
    if not target or not GetPlayerName(target) then return respond({ ok = false, message = 'Player not found' }) end
    TriggerClientEvent('vmenu_ui:teleportToCoords', target, payload.coords)
    return respond({ ok = true, message = 'Player summoned' })
  end

  if action == 'killPlayer' then
    if not hasAce(src, 'vMenu.OnlinePlayers.Kill') and not hasAce(src, 'vMenu.Staff') then return respond({ ok = false, message = 'No permission' }) end
    local target = tonumber(payload.target)
    if not target or not GetPlayerName(target) then return respond({ ok = false, message = 'Player not found' }) end
    TriggerClientEvent('vmenu_ui:killMe', target)
    return respond({ ok = true, message = 'Player killed' })
  end

  if action == 'kickPlayer' then
    if not hasAce(src, 'vMenu.OnlinePlayers.Kick') and not hasAce(src, 'vMenu.Staff') then return respond({ ok = false, message = 'No permission' }) end
    local target = tonumber(payload.target)
    if not target or not GetPlayerName(target) then return respond({ ok = false, message = 'Player not found' }) end
  ...
  if action == 'tempBanPlayer' or action == 'permBanPlayer' then
    local ace = action == 'tempBanPlayer' and 'vMenu.OnlinePlayers.TempBan' or 'vMenu.OnlinePlayers.PermBan'
    if not hasAce(src, ace) and not hasAce(src, 'vMenu.Staff') then return respond({ ok = false, message = 'No permission' }) end
    local target = tonumber(payload.target)
    if not target or not GetPlayerName(target) then return respond({ ok = false, message = 'Player not found' }) end
    local entry = {
